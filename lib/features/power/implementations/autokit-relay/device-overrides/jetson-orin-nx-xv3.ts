...
export class JetsonOrinNxXv3 extends AutokitRelay {
    ...
    async setup(): Promise<void> {
        console.log(`Power button Relay ID: ${this.pwrBtnRelay.powerRelay.relayId} is on HID path: ${this.pwrBtnRelay.powerRelay.devicePath}, channel: ${this.pwrBtnRelay.powerRelay.relayNum}`)
    }
    ...
    async on(): Promise<void> {
        ...
        await this.powerRelay.on(); // toggles the power button relay to high to simulate the button press
        await delay(3000); // wait to simulate the period the button is held down
        await this.powerRelay.off() // toggles the power button relay to high to simulate letting go of the button
